...
type
  ...
  private
    ...
    procedure ClearImageInternal();
    procedure SaveImageAsFile(bitMap: TBitmap);
...
begin
   ...
   DBmod.frReport1.LoadFromFile('..\reports\ColorPrice.frf');
...
begin
    if DBmod.TGDS_DTLImageType.Value = 0 then
    begin
...
begin
   ...
   SaveImageAsFile(ImageEditorForm.imageDst.Picture.Bitmap);
   ImageEditorForm.Dispose();
end;
...
procedure TPriceForm.DeleteImageFile();
var
    imageFile: string;
begin
    imageFile := '..\images\' + DBMod.TGDS_DTLID_GDS_DTL.AsString + '.jpg';
...
begin
    ...
    try
        ...
        begin
            ...
            if DBmod.QGroupsWebDEL.Value = 1 then
                deleted := true
            else
                deleted := false;
            ...
            begin
                if DBmod.QSubgroupsWebDEL.Value = 1 then
                    deleted := true
                else
                    deleted := false;
    ...
    cloudUploaderExePath := '..\CloudUploader\Altech.CloudUploader.exe';
...
procedure TPriceForm.resetSearchButtonClick(Sender: TObject);
begin
  DbEditEh1.Text := '';
  DbEditEh2.Text := '';
end;
